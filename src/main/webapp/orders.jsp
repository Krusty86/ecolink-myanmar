...
<div class="container py-4">
    ...
    <h2 class="fw-bold text-dark d-flex align-items-center">
        <i class="bi bi-cart-check-fill text-success me-2"></i> Order Management
    </h2>
</div>
        <div class="col-md-4">
            <div class="card border-0 shadow-sm rounded-4 bg-white p-3">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <small class="text-muted d-block text-uppercase fw-bold">Total Orders</small>
        ...
        <div class="col-md-4">
            <div class="card border-0 shadow-sm rounded-4 bg-white p-3 border-start border-primary border-4">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <small class="text-muted d-block text-uppercase fw-bold">Active Deliveries</small>
                        <h4 class="fw-bold mb-0">
                            <c:set var="activeCount" value="0" />
                            <c:forEach var="o" items="${orders}">
                                <c:if test="${o.status == 'SHIPPING'}"><c:set var="activeCount" value="${activeCount + 1}" /></c:if>
                            </c:forEach>
                            ${activeCount}
                        </h4>
                    </div>
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <h4 class="fw-bold mb-0">
                            ...
                            <c:forEach var="o" items="${orders}">
                                <c:if test="${o.status == 'PENDING'}"><c:set var="pendingCount" value="${pendingCount + 1}" /></c:if>
                            </c:forEach>
                            ${pendingCount}
                        </h4>
                    </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
        <div class="card-header bg-white py-3 border-0">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="badge bg-light text-dark border fw-normal p-2 rounded-pill">
                ...
                <tbody>
                    <c:forEach var="order" items="${orders}" varStatus="count">
                        <tr>
                            ...
                            <td>
                                <div class="fw-bold text-success">${order.total_amount} MMK</div>
                                <c:if test="${order.discount_amount_from_points > 0}">
                                    <small class="text-danger">(-${order.discount_amount_from_points} discount)</small>
                                </c:if>
                            </td>
                            <td>
                                <c:choose>
                                    <c:when test="${order.status == 'PENDING'}">
                                        <span class="badge bg-warning-subtle text-warning px-3 rounded-pill border border-warning-subtle">Pending</span>
                                    </c:when>
                                    <c:when test="${order.status == 'CONFIRMED'}">
                                        <span class="badge bg-info-subtle text-info px-3 rounded-pill border border-info-subtle">Confirmed</span>
                                    </c:when>
                                    <c:when test="${order.status == 'SHIPPING'}">
                                        <span class="badge bg-primary-subtle text-primary px-3 rounded-pill border border-primary-subtle">Shipping</span>
                                    </c:when>
                                    <c:when test="${order.status == 'DELIVERED'}">
                                        <span class="badge bg-success-subtle text-success px-3 rounded-pill border border-success-subtle">Delivered</span>
                                    </c:when>
                                    <c:otherwise>
                                        <span class="badge bg-light text-muted px-3 rounded-pill border">${order.status}</span>
                                    </c:otherwise>
                                </c:choose>
                            </td>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow rounded-4">
            <div class="modal-header border-0 bg-success py-3">
                <h5 class="fw-bold mb-0 text-white"><i class="bi bi-arrow-repeat me-2"></i>Update Order Status</h5>
            ...
            <form action="home" method="post">
                <div class="modal-body p-4">
                    ...
                    <div class="mb-0">
                        <label class="form-label small fw-bold text-muted text-uppercase">New Status for Order</label>
                        <select class="form-select rounded-3 p-3 shadow-sm" name="status" id="orderStatus" required>
                ...
                <div class="modal-footer border-0 p-4 pt-0">
                    ...
                    <button type="submit" class="btn btn-success rounded-pill px-5 shadow-sm">Save Update</button>
                </div>
            </form>
        </div>
    </div>
